/**
 * /sitemap/cards-pokemon.xml — image: タグ付き + 動的 priority/lastmod
 *
...
const HIGH_RARITY = new Set([
  "SAR", "UR", "SSR", "CHR", "AR", "HR", "SR", "RR", "ACE",
]);
...
export async function GET() {
  let entries: Entry[] = [];
  try {
    // 1. card_grade_prices_latest から PSA 価格データのある card_id を集める
    ...
    // 2. 全カード variant を取得し、(set_code, card_no) でグルーピング
    const PAGE = 1000;
    ...
    // priority 高い順に並べる (Google は順序もシグナルとして見るため)
