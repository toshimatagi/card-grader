...
// ポケカの主要レアリティを高額になりやすい順に並べる (検索流入用のセクション見出し)
const POKEMON_RARITY_PRIORITY = [
  "SAR", "UR", "SSR", "CHR", "AR", "HR", "SR", "RR", "ACE", "R", "U", "C", "PROMO", "PR",
];

function groupByRarity(groups: Group[]): Array<{ rarity: string; cards: Group[] }> {
  const map = new Map<string, Group[]>();
  for (const g of groups) {
    // variant 横断で 1 種類に絞る (代表レアリティ = 最初の rarities[0])
  ...
  const knownOrder = new Set(POKEMON_RARITY_PRIORITY);
  for (const r of POKEMON_RARITY_PRIORITY) {
...
export async function generateStaticParams() {
  return Object.keys(POKEMON_SETS).map((set) => ({ set: set.toUpperCase() }));
}
...
export async function generateMetadata({
  ...
  if (!meta) {
    return {
      title: `${setCode} - ポケカ価格DB | TCG Authority`,
      alternates: { canonical: `/cards/pokemon/${setCode}` },
    };
  }
  const title = `ポケカ「${meta.name}」(${setCode}) 全カード相場 - ${meta.releaseDate}発売`;
  const description = `ポケモンカード「${meta.name}」(${setCode}, ${meta.releaseDate}発売・${meta.kind}) の全カード相場・買取価格・値上がり情報。販売中央値と買取相場を一覧でチェック。`;
  return {
    title,
    description,
    keywords: [
...
export default async function PokemonSetPage({
  ...
  if (!meta) {
    // POKEMON_SETSに無くても DBにデータがあれば表示する。両方無ければ 404。
    const probe = await searchCards({ brand: "pokemon", set_code: setCode, limit: 1 }).catch(
      () => ({ items: [], count: 0 }),
    );
  ...
  // レアリティ別ブロック (高額帯のみ表示。表示する Group は best_sell の高い順)
  ...
  const featuredRarities = ["SAR", "UR", "SSR", "AR", "HR", "SR", "CHR"];
  const featuredBlocks = rarityBlocks.filter((b) => featuredRarities.includes(b.rarity));

  // レアリティ件数 (冒頭文 + FAQ 用)
  const sarCount = rarityBlocks.find((b) => b.rarity === "SAR")?.cards.length ?? 0;
  const urCount = rarityBlocks.find((b) => b.rarity === "UR")?.cards.length ?? 0;
  const srCount = rarityBlocks.find((b) => b.rarity === "SR")?.cards.length ?? 0;
  const arCount = rarityBlocks.find((b) => b.rarity === "AR")?.cards.length ?? 0;

  // 前後弾ナビ (releaseDate 順)
  const neighbors = getNeighborSets(setCode);
  const prevMeta = neighbors.prev ? POKEMON_SETS[neighbors.prev] : null;
  const nextMeta = neighbors.next ? POKEMON_SETS[neighbors.next] : null;
  ...
  const faq = [
    {
      ...
      a: meta
        ? `「${setName}」は ${releaseDate} に発売されたポケモンカードゲームの${kind}です。`
        : `「${setCode}」の発売日情報は近日追加予定です。`,
    },
    {
      q: `${setCode} の全カードリストはどこで確認できますか?`,
      a: `本ページに ${setCode} 全カード (${groups.length}件) の型番・カード名・販売中央値・買取価格を一覧で掲載しています。型番をクリックするとバリアント別 (通常/SAR/UR/SR/AR) の詳細価格を確認できます。`,
    },
    {
      q: `${setCode} で高額になりやすいカードは?`,
      a:
        topCards.length > 0
          ? `現時点で ${setCode} の販売中央値が高いカードは「${topCards[0].name_ja}」(¥${topCards[0].best_sell?.toLocaleString()})${topCards.length > 1 ? `、「${topCards[1].name_ja}」(¥${topCards[1].best_sell?.toLocaleString()})` : ""} などです。SAR・UR・SRなどのレアリティが高額になりやすい傾向があります。`
          : `SAR (スペシャルアートレア)・UR (ウルトラレア)・SR (スーパーレア) のカードが高額になりやすい傾向があります。具体的な相場は本ページのカードリストをご確認ください。`,
    },
    {
      q: `${setCode} の価格はどのくらいの頻度で更新されますか?`,
      a: meta && ["M01", "M01B", "M02", "M02A", "M03", "M04"].includes(setCode)
        ? `現役のMEGAシリーズ (${setCode}) は1時間おきに自動更新しています。販売中央値・買取価格・値上がりトレンドをリアルタイムで反映します。`
  ...
  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: `ポケカ「${setName}」(${setCode}) 全カード相場`,
    description: `「${setName}」(${setCode}) の全カード ${groups.length} 件の相場・買取価格・トレンド一覧`,
    url: `${SITE_URL}/cards/pokemon/${setCode}`,
    isPartOf: {
      "@type": "WebSite",
      name: "TCG Authority",
      url: SITE_URL,
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: "価格DB", item: `${SITE_URL}/cards` },
        {
          ...
          position: 3,
          name: "ポケカ",
          item: `${SITE_URL}/cards/pokemon`,
        },
        {
          "@type": "ListItem",
          position: 4,
          name: setCode,
          item: `${SITE_URL}/cards/pokemon/${setCode}`,
        },
      ],
    },
    mainEntity: {
  ...
  return (
    <div>
      ...
      <nav className="text-xs text-gray-500 mb-2">
        ...
        <Link href="/cards/pokemon" className="hover:underline">
          ポケモンカード
        </Link>
        <span className="mx-1.5">/</span>
        <span>{setCode}</span>
      ...
      <header className="mb-6">
        ...
        <div className="flex flex-wrap gap-2 text-xs text-gray-600 mb-3">
          ...
          <span className="px-2 py-1 bg-yellow-50 text-yellow-900 rounded">
            登録 {groups.length} 件
          </span>
        </div>
        <p className="text-sm text-gray-700 leading-relaxed">
          ポケモンカード「{setName}」({setCode}) の全カード{groups.length}件の相場・買取価格・PSA10想定価格を一覧表示。
          {(sarCount > 0 || urCount > 0 || arCount > 0 || srCount > 0) && (
            <>
              {" "}内訳: {sarCount > 0 && <>SAR {sarCount}枚 / </>}
              {urCount > 0 && <>UR {urCount}枚 / </>}
              {arCount > 0 && <>AR {arCount}枚 / </>}
          ...
          {" "}各型番をクリックすると、通常・SAR・UR・SRなどバリアント別の詳細価格と過去90日の推移グラフを確認できます。フリマ購入・PSA提出前のチェックにご利用ください。
        </p>
      </header>
      ...
      {/* レアリティ別ランキング (検索流入用: 「[弾名] SAR一覧」「[弾名] UR一覧」) */}
            ...
            {featuredBlocks.map((block) => (
              <div key={block.rarity} id={`rarity-${block.rarity}`}>
                <h3 className="text-sm font-bold mb-2 flex items-baseline gap-2">
                  <span className="px-2 py-0.5 bg-yellow-200 text-yellow-900 rounded text-xs font-bold">
                    {block.rarity}
                  </span>
        ...
        <section className="mt-8 border-t pt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {prevMeta && neighbors.prev && (
              <Link
                href={`/cards/pokemon/${neighbors.prev}`}
                className="border rounded p-3 hover:shadow-sm hover:bg-yellow-50 transition-colors"
              >
                <div className="text-xs text-gray-500 mb-0.5">← 前の弾</div>
                <div className="text-sm font-bold">
                  <span className="font-mono text-yellow-900 mr-2">
                    {neighbors.prev}
                  </span>
                  {prevMeta.name}
                </div>
            ...
            {nextMeta && neighbors.next && (
              <Link
                href={`/cards/pokemon/${neighbors.next}`}
                className="border rounded p-3 hover:shadow-sm hover:bg-yellow-50 transition-colors sm:text-right"
              >
      ...
      <section className="mt-8 text-xs text-gray-500 border-t pt-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link
            href="/trending?brand=pokemon"
            className="px-3 py-1 border rounded hover:bg-gray-50"
          >
            ポケカ値上がりランキング
          </Link>
          <Link
            href="/trending/psa10?brand=pokemon"
            className="px-3 py-1 border rounded hover:bg-gray-50"
          >
            ポケカ PSA10価格ランキング
          </Link>
